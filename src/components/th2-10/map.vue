    ...
    l-map(ref='myMap' :zoom='zoom' :center='center' :options='options' style='height: 100vh;')
      ...
      l-tile-layer(:url='url' :attribution='attribution')
        ...
        l-marker(:lat-lng='item.local' v-for='item in data' :key='item.id')
          //- //  標記點樣式判斷 
          l-icon(:icon-url='item.name === \'夢時代購物中心\'?icon.type.gold:icon.type.black' :shadow-url='icon.shadowUrl' :icon-size='icon.iconSize' :icon-anchor='icon.iconAnchor' :popup-anchor='icon.popupAnchor' :shadow-size='icon.shadowSize')
            //- //  彈出視窗 
            l-popup.
